...
from triage.topics import classify_topic, passes_topic_precision_gate, score_topics
...
def filter_relevant_papers(
    ...
    for paper, text, emb in zip(papers, texts, embeddings):
        ...
        topic_precision_passed = passes_topic_precision_gate(paper, best_topic)
        ...
        enriched["topic_precision_passed"] = topic_precision_passed
    ...
    fallback = [
        ...
        and paper["topic_precision_passed"]
        and (
    ...
    if fallback:
        logger.info("Filter fallback activated: no papers met the strict threshold, keeping top %s candidates.", len(fallback))
